...
st.set_page_config(page_title="Energy Viewer v1.6.2", layout="wide")
st.title("Energy Consumption Viewer — v1.6.2")
st.caption("Resilient loading + English titles + legend toggle (default OFF).")

DB_PATH = "data.db"  # stored in app working dir
...
with tab_day:
    if df.empty:
        ...
        # choose nearest available date by default
        ...
        if day:
            ...
            if df_day.empty:
                st.warning("No records for the selected date. Try another date from the list above.")
            else:
